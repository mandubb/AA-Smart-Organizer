"""
AA Smart Organizer v2.0 - Enhanced GUI Application
Modern desktop app with profiles, quick clean, and advanced features
"""
...
class AASmartOrganizerV2(ctk.CTk):
    """Main application window with v2.0 features"""
    
    def __init__(self):
        super().__init__()
        
        # Configure window
        self.title("AA Smart Organizer v2.0 - Professional Edition")
    ...
    def _initialize_components(self):
        ...
        # Get current profile config path
        ...
        self._log_message(f"✅ Loaded profile: {self.profile_manager.get_current_profile()}")
    ...
    def _create_ui(self):
        ...
        title_label = ctk.CTkLabel(
            title_frame,
            text="🗂️ AA Smart Organizer v2.0",
        ...
        subtitle_label = ctk.CTkLabel(
            title_frame,
            text="Professional File Organization & Maintenance System",
        ...
        profile_label.pack(side="left", padx=(15, 10), pady=10)
        ...
        self.profile_dropdown.pack(side="left", padx=5, pady=10)
        ...
        self.new_profile_button.pack(side="left", padx=5, pady=10)
        ...
        self.refresh_profiles_button.pack(side="left", padx=5, pady=10)
        ...
        self.log_textbox.insert("1.0", "Welcome to AA Smart Organizer v2.0!\n")
        self.log_textbox.insert("end", "✨ New: Profile management, Quick Clean, Smart Summaries\n")
        ...
        footer_label = ctk.CTkLabel(
            self,
            text="AA's Computer and Remote Services | v2.0 Professional Edition",
    ...
    def _create_new_profile(self):
        ...
        if profile_name:
            if self.profile_manager.create_profile(profile_name):
                self._log_message(f"✅ Created profile: {profile_name}")
                self._refresh_profiles()
            else:
                self._log_message(f"❌ Failed to create profile: {profile_name}")
    
    def _switch_profile(self, profile_name):
        """Switch to a different profile"""
        if self.profile_manager.load_profile(profile_name):
            self._initialize_components()
            self._log_message(f"✅ Switched to profile: {profile_name}")
        else:
            self._log_message(f"❌ Failed to switch to profile: {profile_name}")
